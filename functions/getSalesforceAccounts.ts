...
Deno.serve(async (req) => {
        try {
          ...
          const [accountRes, saRes, sliRes] = await Promise.all([
            fetch(`https://adtrak.salesforce.com/services/data/v60.0/query?q=${encodeURIComponent(accountQuery)}`, {
              headers: { 'Authorization': `Bearer ${accessToken}`, 'Content-Type': 'application/json' }
            }),
            fetch(`https://adtrak.salesforce.com/services/data/v60.0/query?q=${encodeURIComponent(serviceAgreementQuery)}`, {
              headers: { 'Authorization': `Bearer ${accessToken}`, 'Content-Type': 'application/json' }
            }),
            fetch(`https://adtrak.salesforce.com/services/data/v60.0/query?q=${encodeURIComponent(subscriptionLineItemQuery)}`, {
              headers: { 'Authorization': `Bearer ${accessToken}`, 'Content-Type': 'application/json' }
            })
          ]);

          if (!accountRes.ok || !saRes.ok || !sliRes.ok) {
            return Response.json({ error: 'Failed to fetch Salesforce data' }, { status: 500 });
          }
